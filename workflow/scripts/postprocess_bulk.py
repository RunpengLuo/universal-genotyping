...
if "normal" in rep_ids:
    ...
    dp_mat_files[0], dp_mat_files[normal_idx] = (
        dp_mat_files[normal_idx],
        dp_mat_files[0],
    )
...
for i in range(len(rep_ids)):
    dp_canon, ad_canon = canon_mat_one_replicate(
        parent_keys, vcf_files[i], dp_mat_files[i], ad_mat_files[i], ncells=1
    )
...
if min_depth > 0:
    snp_mask = snp_mask & get_mask_by_depth(snps, dp_mtx, min_dp=min_depth)
if "normal" in rep_ids:
...
np.savez_compressed(sm.output["dp_mtx"], mat=dp_mtx)
