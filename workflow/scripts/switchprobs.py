...
def interp_cM_snps(snp_info: pd.DataFrame, genetic_map: pd.DataFrame):
    """Interpolate per-SNP cM positions and compute consecutive inter-SNP cM distances.

    The first SNP on each chromosome receives a distance of 0.

    Parameters
    ----------
    snp_info : pd.DataFrame
        SNP DataFrame with ``#CHR`` and ``POS`` columns. Modified in-place
        with a ``dist_cM`` column.
    genetic_map : pd.DataFrame
        Genetic map with ``#CHR``, ``POS``, and ``cM`` columns.
