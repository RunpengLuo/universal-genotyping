##################################################
# genotype SNPs from matched bulk normal samples
rule genotype_snps_bulk_normal:
    ...
    output:
        done_file="snps/.bulk_normal.chr{chrname}.done",
...
##################################################
# genotype SNPs from pseudobulk tumor samples
...
rule annotate_snps_pseudobulk:
    ...
    output:
        done_file=expand("snps/.pseudobulk_tumor.chr{chrname}.done", chrname=config["chromosomes"]),
    ...
    threads: 1
    log:
        "logs/annotate_snps.pseudobulk_tumor.log"
    script:
