...
if workflow_mode == "single_cell":
    rule postprocess_single_modality:
        input:
            ...
            block_bed=lambda wc: branch(config.get("block_bed") is None, then=[], otherwise=config["block_bed"]),
            seg_ucn = lambda wc: branch(config.get("seg_ucn") is None, then=[], otherwise=config["seg_ucn"]),
            bbc_ucn = lambda wc: branch(config.get("bbc_ucn") is None, then=[], otherwise=config["bbc_ucn"]),
            bbc_phases = lambda wc: branch(config.get("bbc_phases") is None, then=[], otherwise=config["bbc_phases"]),
        output:
