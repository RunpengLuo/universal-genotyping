## Unified genotyping pipeline (Bulk Mode)
### Input
* matched normal, tumor BAM files
* SNP panel
* phase panel
* gene GTF file.

### Output
* phased A/B allele count matrices, segment by sample
* RDR matrix, segment by sample.
* segment meta-data.


### Steps
* parse genetic map file.
* per chromosome, use SNP positions from SNP panel to pileup and genotype Hom/Het SNPs via bcftools using matched normal BAM file.
* phase and extract phased Het SNPs.
* pileup allele read counts given phased Het SNPs and BAM files.
* 

### QC
* after SNP genotyping, annotate SNP with gene labels?
...
4. `postprocess_bulk` takes phased Het SNPs and allele count vectors, and additional BED files including following:
    ...
    2. `gtf_file`: BED file contains non-overlapping aggregation unit blocks like gene blocks, aggregated bins cannot contain partial blocks.

    And do the following:
    1. concat per-sample count vectors into SNP by sample count matrices.
    2. filter SNPs based on `region_bed`, per-sample min-depth, beta-posterior credible interval AF test on normal sample. If `gtf_file` is given, filter SNPs not in `gtf_file`.
