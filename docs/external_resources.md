...
### TODO
1. preprocess 10x feature data to cnp-level mats when cnp available.
2. multiome adaptive binning when no bulk data?
3. gencode may overlap, convert to non-overlapping units and keep as a file in resources.
4. use non-overlapping gencode to guide adaptive binning step?
5. mapping resolutions, 2um to Xum?
6. Detecting clonal LOH, from Numbat, HetDetect `https://github.com/raphael-group/hetdetect.git`
```
